...
# used for training
def early_exit_loss(outputs, target, criterion, num_early_exit: int, early_exit_loss_weights: list):
    loss = 0.
    sum_loss_weights = 0.

    for i in range(num_early_exit): # TODO: 现在只有 1 个早退点
        loss += early_exit_loss_weights[i] * criterion(outputs[i], target)
        sum_loss_weights += early_exit_loss_weights[i]
        # args.exiterrors[exitnum].add(output[exitnum].data, target)

    loss += (1. - sum_loss_weights) * criterion(outputs[-1], target)
...
def train(train_loader, model, criterion, optimizer, epochs):
    ...
    for epoch in range(epochs):
        for (inputs, target) in train_loader:
            ...
            loss = early_exit_loss(outputs, target, criterion)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
